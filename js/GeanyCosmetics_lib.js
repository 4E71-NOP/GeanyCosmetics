...
geany_fold_symbol_highlight:{ id:'geany_fold_symbol_highlight', text:'fold_symbol_highlight', on:0, display:'0', ObjClass:0, ObjDivFg:0, ObjDivBg:0, ObjChkBold:0, ObjChkItalic:0 },
...
geany_keyword:{ id:'geany_keyword', text:'keyword', on:1, display:'1', ObjClass:0, ObjDivFg:0, ObjDivBg:0, ObjChkBold:0, ObjChkItalic:0 },
...
geany_keyword_2:{ id:'geany_keyword_2', text:'keyword_2', on:0, display:'0', ObjClass:0, ObjDivFg:0, ObjDivBg:0, ObjChkBold:0, ObjChkItalic:0 },
...
function formatSampleText ( content , target ){
	var i=1, elm, elm2, tabMatch, tabMatch2, strRegexp1, strRegexp2, str2="100101" , str1 = content;

	for ( elm in HTMLCompatTab ){
		str1 = str1.replace( HTMLCompatTab[elm].m , HTMLCompatTab[elm].r );
	}

	for ( elm in regexpClassTab ){
		if ( regexpClassTab[elm].convert == 1 ) { 					//Converting when forging RegExp. Because even String.raw won't help as it doesn't accepts variable... We're still forced to double backslashes
			regexpClassTab[elm].reg = escapeRegExp(regexpClassTab[elm].prepared);
			regexpClassTab[elm].convert = 0;
			tabJSJournal[tabJSJournalSw]("reg:"+regexpClassTab[elm].reg+"; original:"+regexpClassTab[elm].prepared );
		}
		strRegexp1 = new RegExp(regexpClassTab[elm].reg , regexpClassTab[elm].f );
		...
		for ( elm2 in tabMatch ) { 
			if ( tabMatch[elm2].length >0) {
				...
				str1 = str1.replace (strRegexp2, "<span id='spanPHP"+i+"'class='"+regexpClassTab[elm].cl+"'>" +tabMatch[elm2]+"</span>");
				i++;
			}
		}
	}
	...
	for ( elm=1 ; elm<i ; elm++ ) {
		Gebi("spanPHP"+elm).onclick = function () { OpenColorSelector('.'+this.className, 'fg'); };
	}
}
